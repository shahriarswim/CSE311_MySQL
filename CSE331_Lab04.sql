...
WHERE departments.Location_id = locations.Location_id AND employees.Manager_id = departments.Manager_id AND employees.Commission_pct IS NOT NULL;

-- Task 02

-- Activity 01

SELECT employees.Last_Name, departments.Department_id, departments.Department_Name, jobs.job_title, locations.Street_Address 
FROM employees, departments, jobs, locations 
WHERE employees.Department_id = departments.Department_id AND locations.Location_id = departments.Location_id AND employees.Job_Id = jobs.job_id AND locations.City ='Toronto';
...
SELECT e.Last_Name as 'Employess', e.Employee_Id as 'EMP#', m.Last_Name as 'Manager', m.Employee_Id as 'Mgr#' 
